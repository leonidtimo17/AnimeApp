...
from .player.kodik_page import KodikPage
from .player.window import PlayerWindow
from .theme import ACCENT, MUTED
from .widgets.shiki_button import ShikimoriButton
...
class MainWindow(QMainWindow):
    ...
    # ------------------------------------------------------------ playback
    def play(self, release_id, episode_key):
        ...
        def ready(plan):
            ...
            self._launch(plan.release, plan.dub, plan.episodes, plan.key)

        self.ctx.playback.prepare(release_id, episode_key, ready, done)
    ...
    def _switch_dub(self, dub):
        ...
        def ready(plan, note):
            ...
            self._launch(release, dub, plan.episodes, plan.key, position=plan.position)

        self.ctx.playback.switch_dub(release, dub, key, pos, ready,
                                     lambda msg: QMessageBox.warning(self, APP_NAME, msg))
